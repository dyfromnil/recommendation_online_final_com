...
from .util.dissim import matching_dissim, euclidean_dissim, new_dissim
...
def _labels_cost(Xnum, Xcat, centroids, num_dissim, cat_dissim, gamma, membship=None):
    ...
    for ipoint in range(n_points):
        ...
        tot_costs = num_costs + gamma * np.array(cat_costs)
...
def _k_prototypes_iter(Xnum, Xcat, centroids, cl_attr_sum, cl_memb_sum, cl_attr_freq,
    ...
    for ipoint in range(Xnum.shape[0]):
        clust = np.argmin(
            ...
            gamma * np.array(cat_dissim(centroids[1], Xcat[ipoint], X=Xcat, membship=membship))
        )
    ...
    while True:
        ...
        for ipoint in range(n_points):
            ...
            clust = np.argmin(
                ...
                np.array(cat_dissim(centroids[1], Xcat[ipoint], X=Xcat, membship=membship))
            )
...
class KPrototypes(kmodes.KModes):
    ...
    Attributes
    ----------
    data_: array, [n_points, n_features]
    ...
    def __init__(self, n_clusters=8, max_iter=100, num_dissim=euclidean_dissim,
                 cat_dissim=new_dissim, init='Huang', n_init=10, gamma=None,
    ...
    def get_nearest(self, X, categorical=None):
        '''
        ...
        '''
        result = []
        labels, cluster_points = self.get_similar_by_points(X, categorical)
        for index, sample in enumerate(X):
            cost = {}
            sample_num = list()
            sample_cat = list()
            sample_num.append(sample[2])
            sample_num.append(sample[3])
            sample_cat.append(sample[0])
            sample_cat.append(sample[1])
            
            cluster_point = cluster_points[index]
            sim_data = []
            for i in cluster_point:
                sim_data.append(self.data_[i])
            Xnum, Xcat = _split_num_cat(np.array(sim_data), categorical)
            for ipoint, npoint in enumerate(cluster_point):
                '''
                计算数值欧式距离
                '''
                num_costs = (sample_num[0]-Xnum[ipoint][0]) ** 2 + (sample_num[1]-Xnum[ipoint][1]) ** 2 
                '''
                计算分类距离
                '''
                cat_costs = 0
# =============================================================================
#                 if sample_cat[0] != Xcat[ipoint][0]:
#                     cat_costs += 1
#                 if str(sample_cat[1])[0] == str(Xcat[ipoint][1])[0]:
#                     if str(sample_cat[1])[1] == str(Xcat[ipoint][1])[1]:
#                         if str(sample_cat[1])[2] == str(Xcat[ipoint][1])[2]:
#                             if str(sample_cat[1])[3] == str(Xcat[ipoint][1])[3]:
#                                 cat_costs += 0
#                             else:
#                                 cat_costs += 0.25
#                         else:
...
# =============================================================================
                if sample_cat[0] != Xcat[ipoint][0]:
                    cat_costs += 1
                if str(sample_cat[1])[0:2] == str(Xcat[ipoint][1])[0:2]:
                    if str(sample_cat[1])[2:4] == str(Xcat[ipoint][1])[2:4]:
                        cat_costs += 0.
                    ...
                    cat_costs += 1
                # Gamma relates the categorical cost to the numerical cost.
                tot_costs = num_costs + self.gamma * cat_costs
                cost[npoint] = tot_costs
            sorted_cost = sorted(cost.items(), key=lambda d:d[1]) 
            sorted_near = []
            for sample in sorted_cost:
                sorted_near.append(sample[0])
            result.append(sorted_near)
    ...
    def get_similar_by_label(self, nlabel):
        '''
        get all points in this label
        '''
        result = []
    ...
    def get_similar_by_points(self, X, categorical=None):
        '''
        预测样本的类别，并找出该类别所有其他样本
        '''
        result = []
        temp = []
        labels = self.predict(X, categorical)
        for nlabel in labels:
            for index, label in enumerate(self.labels_):
                if label == nlabel:
                    temp.append(index)
            result.append(temp)
    ...
    def cluster_centroids_(self):
        if hasattr(self, '_enc_cluster_centroids'):
            return [
                self._enc_cluster_centroids[0],self._enc_cluster_centroids[1]
